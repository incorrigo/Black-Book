...
public static class SessionManager {
    ...
    public static bool LoadSession (string name, string password) {
        ...
        EncryptedContainerManager.SaveEncrypted(SessionManager.Data!, name);

        CurrentUserName = name;
        Certificate = cert;
        Data = EncryptedContainerManager.LoadEncrypted(name);
        Data.LastOpened = DateTime.UtcNow;
        Data.AccessCount++;
        EncryptedContainerManager.SaveEncrypted(Data, name);
